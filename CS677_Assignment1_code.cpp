...
float interpolate(float x[], float y[], float a) {
    ...
    int i = 0;
    while (i < n - 1) {
        float interval_start = x[i];     
        float interval_end = x[i + 1];  
        float value_start = y[i];       
        float value_end = y[i + 1];     
...
float opacityFunction(float a){

    float intensity2[] = {-4926.59, -1310.07, 103.00, 2593.85};
    float opacity[] = {1.0, 0.75, 0.0, 0.05};
...
// std::vector<float> colorTrialFunction(float a,float intensity[],float R[],float G[],float B[]) {
...
std::vector<unsigned char> colorTrialFunction(float a, float intensity[], float R[], float G[], float B[]) {
    int n = 55; // Number of elements in intensity array

    // Handle edge cases: return first or last color if 'a' is out of bounds
    if (a <= intensity[0]) 
        return std::vector<unsigned char>({static_cast<unsigned char>(std::round(R[0] * 255)), 
                                   static_cast<unsigned char>(std::round(G[0] * 255)), 
                                   static_cast<unsigned char>(std::round(B[0] * 255))});

    if (a >= intensity[n - 1]) 
        return std::vector<unsigned char>({static_cast<unsigned char>(std::round(R[n - 1] * 255)), 
                                   static_cast<unsigned char>(std::round(G[n - 1] * 255)), 
                                   static_cast<unsigned char>(std::round(B[n - 1] * 255))});

   //Interpolate for each of the RGB channels
    float red = interpolate2(intensity, R, a);
    float green = interpolate2(intensity, G, a);
    float blue = interpolate2(intensity, B, a);

    // Return the interpolated color as a vector of uchar values (0 to 255)
    return std::vector<unsigned char>{
        static_cast<unsigned char>(std::round(red * 255)), 
        static_cast<unsigned char>(std::round(green * 255)), 
        static_cast<unsigned char>(std::round(blue * 255))
    };
...
std::tuple<int, int, int> calculateIndices(int x, int y, float z, int yDimension, int xDimension) {
    // Compute the base index
    int baseIndex = x + y * xDimension;
    ...
    int fullIndex = baseIndex + integerZ * yDimension * xDimension;

    return std::make_tuple(fullIndex, integerZ, nextZ);

}

void accumulateRGB(float z, const unsigned char* rgb, float op, float& r, float& g, float& b, float& accumulatedValue) {
...
void Algorithm_rayCasting(int xStart, int xEnd, int yStart, int yEnd,int zDim, int yDim,int xDim,
    ...
    while (x <= xEnd) {
        ...
        while (y <= yEnd) {
            ...
            float z = 0;
            while (z < zDim) {
                auto [idx, zIdx, zNext] = calculateIndices(x, y, z, yDim, xDim);
                if (zNext >= zDim) break; // Avoid accessing out of bounds

                float interpolatedData = interpolate3(z, zIdx, zNext, x, y, yDim, data);

                // int index = static_cast<int>((x / static_cast<float>(800)) * xDim) +
                //             static_cast<int>((y / static_cast<float>(800)) * yDim) * xDim +
                //             static_cast<int>(z) * xDim * yDim;
                
                // float intensity10 = data[index];
                // float op = opacityFunction(intensity10);
                // std::vector<unsigned char> rgb = colorTrialFunction(intensity10, intensity, R, G, B);
               // Vec3b sampleColor = colorTF(intensity);

                std::vector<unsigned char> rgb = colorTrialFunction(interpolatedData, intensity, R, G, B);
                float op = opacityFunction(interpolatedData);
                accumulateRGB(z, rgb.data(), op, r, g, b, accumulatedValue);
                //TERMINATION_THRESHOLD 
                bool terminateEarly = (accumulatedValue >= 0.97);
                if (terminateEarly) {
                    ++earlyTerminatedRays;
            ...
            int linearIndex = (x - xStart)  + (y - yStart)*xSize;
...
void MergeAllResults(int xStart, int yStart, const std::vector<float>& subResult, 
                          std::vector<float>& finalResult, int xBoundMin,int xBoundMax, int yBoundMin, 
    ...
    while (i < xSize) {
        ...
        while (j < ySize) {
            ...
            int boundingBoxWidth = xBoundMax - xBoundMin + 1;

            int xOffset = xStart - xBoundMin + i;

            // Compute the offset in the y direction from the bounding box minimum
            int yOffset = yStart - yBoundMin + j;

            // Compute the final index based on x and y offsets and multiply by 3
            int finalOffset = xOffset + (yOffset * boundingBoxWidth);
            int finalIndex = finalOffset * 3;

            // Compute the index for the sub-result array
            int subOffset = i + j * xSize;
            int subIndex = subOffset * 3;
...
void AlgorithmVolumeRendering(MPI_Comm comm, int xDim, int yDim, int zDim, const std::vector<float>& data, 
                     float stepSize, int xBoundMin, int xBoundMax, int yBoundMin, int yBoundMax, 
                     int type, float intensity[], float R[], float G[], float B[]) {
    int rank, size;
    ...
    // 1D partitioning
    if (type == 1) {
        std::tie(xStart, xEnd, yStart, yEnd) = calculateBounds(xBoundMin, xBoundMax, yBoundMin, yBoundMax, rank, size, type);
    }
    // 2D partitioning
    else if (type == 2) {
    ...
    Algorithm_rayCasting(xStart, xEnd, yStart, yEnd, zDim, yDim,xDim, data, stepSize, subResult, earlyTerminatedRays,intensity, R, G, B);
    ...
    if (rank == 0) {
        ...
        MergeAllResults(xStart, yStart, subResult, finalResult, xBoundMin,xBoundMax, yBoundMin, xSize, ySize, zDim);
        ...
        for (int i = 1; i < size; i++) {
            ...
            MergeAllResults(recvXStart, recvYStart, recvSubResult, finalResult, xBoundMin,xBoundMax, yBoundMin, recvXSize, recvYSize, zDim);
        }
    ...
    std::cout << "Rank " << rank << ": Rays terminated early in ratio of total Rays = " << fractionTerminated << std::endl;
    ...
    double time_lcal = elapsed.count();
    double TimeMax;

    MPI_Reduce(&time_lcal, &TimeMax, 1, MPI_DOUBLE, MPI_MAX, 0, comm);

    if (rank == 0) {
        std::cout << "Total time taken: " << TimeMax << " seconds" << std::endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    cout << "Total number of argc: " << argc << endl;
    ...
    if (rank == 0) {
       ...
       AlgorithmVolumeRendering(MPI_COMM_WORLD, xDim, yDim, zDim, data, stepSize, xBoundMin, xBoundMax, yBoundMin, yBoundMax, type,
       intensity, R, G, B);
